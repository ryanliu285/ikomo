...
$sql = 'SELECT * FROM HeightsScore ORDER BY score DESC LIMIT 5';
...
$result = $result -> fetch_object()->played;
  if($result==0){
  $played = FALSE;
  $earnings = $_SESSION['scoreHeights']/2000*25;
  if($earnings < 25){
    $played = TRUE;
  }else{
    $sql = 'SELECT KOMOcoins FROM iKOMODB WHERE idUsers = '.$id.'';
...
<center><h1 id = "daily">YOU EARNED YOUR DAILY '.$earnings.' KOMOCOINS FOR ACHIEVING '.$_SESSION['scoreHeights'].' points</h1></center>
</div>
